               ...
               {activeTab === 'posts' && (
                ...
                {activeTab === 'media' && (
                  <div className='flex flex-wrap mt-6 max-w-6xl'>
                    {
                      posts.filter((posts)=>posts.image_urls.length >0).map((posts)=>(
                        <>
                        {posts.image_urls.map((image , index)=>(
                         <Link target='_blank' to={image} key={index} className='relative group'>
                          <img src={image} key={index} className='w-64 aspect-video object-cover' alt="" />
                          <p className='absolute bottom-0 right-0 text-xs p-1 px-3 backdrop-blur-xl text-white opacity-0 group-hover:opacity-100 transition duration-300'>Posted {moment(posts.createdAt).fromNow()}</p>
                         </Link>
                        ))}
                        </>
                      ))
